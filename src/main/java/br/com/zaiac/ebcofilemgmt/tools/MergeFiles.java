...
*/
  ...
  public static void queue(
    ...
    if (sftpCredentials != null) {
      for (SftpCredentials sftpCredential : sftpCredentials) {
        try {
          if (debugMode) LogApp.writeLineToFile(
            logDirectory,
            Constants.LOGFILE,
            "Adicional Priority (" +
    ...
    try {
      ...
      for (File fileMissing : filesMissing) {
        ...
        try {
          ...
          if (sftpCredentials != null) {
            processTruckIdToSftp(baseDir, missingDir, fileMissing, sftpCredentials, 0, trkId, processStep);
          } else {
            processTruckIdToGcp(
              baseDir,
              missingDir,
      ...
      for (File fileQueue : filesQueue) {
        ...
        if (sftpCredentials != null) {
          ...
          processTruckIdToSftp(baseDir, missingDir, fileQueue, sftpCredentials, 0, trkId, processStep);
        } else {
          try {
            LogApp.writeLineToFile(
              logDirectory,
  ...
  private static String processTruckIdToSftp(
    ...
    try {
      ...
      if (priority > 0) for (int i = 1; i < sftpCredentials.size(); i++) {
      ...
      if (processStep.equalsIgnoreCase("sftp")) currentProcessStepSequence = 1;
      if (processStep.equalsIgnoreCase("deleteDirectory")) currentProcessStepSequence = 2;
  ...
  private static String processTruckIdToGcp(
    ...
    File baseDirFile = new File(baseDir + "\\\\" + trkId);
    System.out.println("fileCopy 1");

    Integer currentProcessStepSequence = 0;
    if (processStep.equalsIgnoreCase("merge")) currentProcessStepSequence = 1;
    if (processStep.equalsIgnoreCase("encryptFile")) currentProcessStepSequence = 2;
    if (processStep.equalsIgnoreCase("uploadObject")) currentProcessStepSequence = 3;
    if (processStep.equalsIgnoreCase("moveFiles")) currentProcessStepSequence = 4;
    ...
    try {
      System.out.println("fileCopy" + baseDirFile.getAbsolutePath());
      File fileCopy = new File(baseDirFile.getAbsolutePath());
      File[] filesCopy = fileCopy.listFiles();
    ...
    try {
      ...
      try {
        LogApp.writeLineToFile(
          logDirectory,
          Constants.LOGFILE,
          "Enqueue Cannot write queue file " + trkId + " to queue directory " + queueDirectory.getAbsolutePath(),
          2
        );
      } catch (WriteLogFileException e1) {
        e.printStackTrace();
    ...
    try {
      ...
      try {
        if (debugMode) LogApp.writeLineToFile(
          logDirectory,
          Constants.LOGFILE,
          "Enqueue Source Directory " + trkId + " with priority " + priority + " enqueued successfully",
          0
        );
      } catch (WriteLogFileException e) {
        System.err.println("Cannot write log file Directory " + logDirectory + " file name " + Constants.LOGFILE);
...
*/
  ...
  public static void checkStepMissingFile(String baseDir, String trkId) throws ProcessIncompleteException {
    ...
    try {
      ...
      throw new ProcessIncompleteException(
        "Cannot write missing file " + trkId + " to missing directory " + missingDirectory.getAbsolutePath()
      );
    }
  }
    ...
    try {
      ...
      try {
        if (debugMode) LogApp.writeLineToFile(
          logDirectory,
          Constants.LOGFILE,
          "Missing Source Directory " + trkId + " enqueued successfully",
          0
        );
      } catch (WriteLogFileException e) {
        System.err.println("Cannot write log file Directory " + logDirectory + " file name " + Constants.LOGFILE);
    ...
    if (xmlConverter.equalsIgnoreCase("CARGO")) {
      ...
      try {
        if (debugMode) LogApp.writeLineToFile(
          logDirectory,
          Constants.LOGFILE,
          String.format("XML CONVERTED - Source -> %s, Destination -> ", fi.getAbsolutePath(), fo.getPath()),
          0
        );
      } catch (WriteLogFileException e) {
        System.err.println("Cannot write log file Directory " + logDirectory + " file name " + Constants.LOGFILE);
...
*/
  ...
  public static boolean checkAllNeedFiles(File[] filesCopy, String dirTruckCurrent) {
    ...
    System.out.println(
      "img: " + img + " xml: " + xml + " jpeg: " + jpeg + " tif: " + tif + " stampJpeg: " + stampJpeg
    );
  ...
  public static boolean checkAllNeedFilesEbco(File[] filesCopy, String dirTruckCurrent) {
    ...
    System.out.println("img: " + img + " xml: " + xml + " jpeg: " + jpeg + " stampJpeg: " + stampJpeg);
